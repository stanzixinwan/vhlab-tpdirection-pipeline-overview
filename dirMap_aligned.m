function dirMap_aligned(S, tcDoc_All, options)
    %DIRMAP_ALIGNED Display direction selectivity map with aligned vectors
    %
    %   DIRMAP_ALIGNED(S, TCDOC_ALL)
    %
    %   Creates a spatial map where the preferred direction at the reference 
    %   temporal frequency (TF index 2, Blue) is rotated to be 0 degrees (East).
    %   The comparison frequency (TF index 4/last, Red) is rotated by the same
    %   amount to show the relative shift.
    %
    %   This visualization helps isolate TF-dependent direction shifts from
    %   spatial retinotopy.
    %
    %   Optional Name-Value Pairs:
    %       'MinDI', value - Minimum Direction Index to include a cell (default: 0.0)
    %                        Helps filter out noisy cells with weak tuning.
    %       'ShowSummary', logical - Plot a summary polar histogram (default: true)
        ...
        for i = 1:max_cells
            ...
            if numel(DI) < idx_low || any(isnan(DI))
                continue
            end
        ...
        try
            ...
            q1 = quiver(X, Y, U1, V1, 'LineWidth', 1, 'Color', [0.6 0.6 0.6], 'MaxHeadSize', 1);

            offset_y = 1; 
            
            % Plot Red arrows (Comparison)
            q2 = quiver(X, Y - offset_y, U2, V2, 'LineWidth', 1.5, 'Color', 'r', 'MaxHeadSize', 1);
