function [pref_resp, theta_pref, TF, DI, SF_used] = dirVector(tcDoc, options)
%DIRVECTOR Calculate preferred direction and response strength for each temporal frequency
%
%   [PREF_RESP, THETA_PREF, TF, DI, SF_used] = DIRVECTOR(TCDOC, OPTIONS)
%
%   Computes direction tuning using responses at the best spatial frequency
%   (the SF with the strongest response). For each temporal frequency, returns
%   preferred direction, response strength, and direction index.
%
%   Inputs:
...
%       SF_used   - The Spatial Frequency value used for each temporal frequency
    arguments
    ...
    [~,best_idx] = max(tc.response_mean); % finds the index of the largest mean response
    best_SF = tc.independent_variable_value(best_idx,2);
    ...
    for i = 1:numel(temporalFrequencies)
        ...
        for j = 1:numel(tc.response_mean)
            ...
            if tc.independent_variable_value(j, 3) == temporalFrequencies(i) && ...
               tc.independent_variable_value(j, 2) == best_SF
                count = count + 1;
